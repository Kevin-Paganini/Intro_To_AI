#
# dungeon3.py: represents a dungeon in the style of Zork.
#
# Author: Robert W. Hasker, 2020
# Edited by: Roby Velez, 2022
#
...
class Room:
    ...
    Attributes:
        ...
        floor (int): level of room, defaulting to 1
    """
    
    def __init__(self, name: str, x: int = 0, y: int = 0, z: int = 1 ):
    ...
    def estimated_cost_to(self, other_room) -> int:
        ...
        #TODO You should use the x, y, and z values of the current
        

        total = abs(self._x - other_room._x) + abs(self._y - other_room._y) + abs(self._z - other_room._z)
    ...
    def __eq__(self, other: Any) -> bool:
        """Returns true if other has the same name"""
...
class Dungeon:
    ...
    def valid_path(self, path: List[Room]) -> bool:
        ...
        while len(copied_path) > 0:
            ...
            if next_room not in [n for n in current.neighbors if n is not None]:
                return False
            current = next_room
        return True
